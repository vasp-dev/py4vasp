...
def nics(raw_data, request):
    return make_nics(raw_data.nics(request.param))


@pytest.fixture
def nics_on_a_grid(raw_data):
    return make_nics(raw_data.nics("on-a-grid"))


@pytest.fixture
def nics_at_points(raw_data):
    return make_nics(raw_data.nics("at-points"))
...
def test_read(nics, Assert):
    actual = nics.read()
    actual_structure = actual.pop("structure")
    Assert.same_structure(actual_structure, nics.ref.structure.read())
    assert actual.keys() == nics.ref.output.keys()
    assert actual["method"] == nics.ref.output["method"]
    Assert.allclose(actual["nics"], nics.ref.output["nics"])
    if "positions" in actual:
        Assert.allclose(actual["positions"], nics.ref.output["positions"])
...
def test_to_numpy(nics, selection, Assert):
    tensor = nics.ref.output["nics"]
    element = get_3d_tensor_element_from_grid(tensor, selection or "3x3")
    Assert.allclose(nics.to_numpy(selection), element)


def test_to_numpy_conventions(nics, Assert):
    tensor = nics.ref.output["nics"]
...
def test_print(nics, format_):
    actual, _ = format_(nics)

...
def test_select_returns_instances(self_energy):
    ...
    selections = self_energy.selections()
    from py4vasp._calculation.electron_phonon_self_energy import ElectronPhononSelfEnergyInstance
    for nbands_sum in selections["nbands_sum"]:
        for selfen_approx in selections["selfen_approx"]:
            selected = self_energy.select(f"nbands_sum({nbands_sum}) selfen_approx({selfen_approx})")
            assert len(selected) == 3
            assert all(isinstance(x, ElectronPhononSelfEnergyInstance) for x in selected)

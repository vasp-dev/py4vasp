...
def run_info(request, raw_data):
    ...
    run_info.ref.band_dispersion_eigenvalues = (
        raw_run_info.band_dispersion_eigenvalues
    )
    ...
    run_info.ref.phonon_dispersion = Dispersion.from_data(raw_run_info.phonon_dispersion)
    return run_info
...
def _check_dict(data_dict, runinfo_ref):
    ...
    # from structure
    assert data_dict["num_ion_steps"] == runinfo_ref.structure._raw_data.positions[:].shape[0]

    # from system
    assert data_dict["system_tag"] == runinfo_ref.system_name

    # from contcar
    assert data_dict["has_selective_dynamics"] == (not check.is_none(runinfo_ref.contcar._raw_data.selective_dynamics))
    assert data_dict["has_ion_velocities"] == (not check.is_none(runinfo_ref.contcar._raw_data.ion_velocities))
    assert data_dict["has_lattice_velocities"] == (not check.is_none(runinfo_ref.contcar._raw_data.lattice_velocities))

    # from phonon dispersion
    assert data_dict["phonon_num_qpoints"] == runinfo_ref.phonon_dispersion._raw_data.eigenvalues[:].shape[0]
    assert data_dict["phonon_num_modes"] == runinfo_ref.phonon_dispersion._raw_data.eigenvalues[:].shape[1]
    ...
    assert data_dict["is_metallic"] == all(runinfo_ref.bandgap._output_gap("fundamental", to_string=False) <= 0.0)

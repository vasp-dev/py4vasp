...
class ParametersInfoFinder(NodeVisitor):
    ...
    def visit_desc_parameterlist(self, node): pass
    ...
    def visit_inline(self, node):
        if (self._next_is_default and self._current_parameter_name):
            if (self._parameters[self._current_parameter_name].get("default") == None):
                self._parameters[self._current_parameter_name]["default"] = node.astext()
            self._next_is_default = False

    def visit_desc_content(self, node): 
        if self._skip_content: raise SkipNode
        pass
    ...
    def _add_param_info_from_node(self, node):
        ...
        for line in lines:
            if " – " in line:
                left,_ = line.split(" – ", 1)
                name, type_, default = _get_param_raw_info_from_left_string(left, _construction_dict = self._parameters)
                if name not in self._parameters:
                    self._parameters[name] = {}

                if (self._parameters[name].get("type") == None): self._parameters[name]["type"] = type_
                if (self._parameters[name].get("default") == None): self._parameters[name]["default"] = default
    ...
    def visit_bullet_list(self, node):
        if not(self._in_parameters_field): raise SkipNode

    def visit_list_item(self, node):
        if not(self._in_parameters_field): raise SkipNode
        else: self._add_param_info_from_node(node)


    


def _get_param_raw_info_from_left_string(left: str, _current_signature_dict: dict={}, _construction_dict: dict={}) -> tuple[str, str, str]:
        ...
        if _current_signature_dict.get("sig_parameters"):
            ...
            for param, _type, default in _current_signature_dict[
                "sig_parameters"
            ]:
                if param == left:
                    sig_default = default

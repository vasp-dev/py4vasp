...
{_slice.examples("magnetism")}
""".strip()


@_documentation.add(_magnetism_docs)
class Magnetism(_slice.Mixin, _base.Refinery):
...
{_slice.examples("magnetism", "read")}"""
    )
    def to_dict(self):
        return {
...
{_slice.examples("magnetism", "plot")}"""
    )
    def plot(self):
        if self._is_slice:
            message = (
...
{_slice.examples("magnetism", "charges")}"""
    )
    def charges(self):
...
{_slice.examples("magnetism", "moments")}"""
    )
    def moments(self):
        moments = _Moments(self._raw_data.moments)
...
{_slice.examples("magnetism", "total_charges")}"""
    )
...
{_slice.examples("magnetism", "total_moments")}"""
    )
